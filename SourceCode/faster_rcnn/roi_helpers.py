...
def calc_iou(R, img_data, class_mapping):
    ...
    for bbox_num, bbox in enumerate(bboxes):
        ...
        gta[bbox_num, 0] = int(round(bbox['x1'] * (resized_width / float(width)) / 16))
        gta[bbox_num, 1] = int(round(bbox['x2'] * (resized_width / float(width)) / 16))
        gta[bbox_num, 2] = int(round(bbox['y1'] * (resized_height / float(height)) / 16))
        gta[bbox_num, 3] = int(round(bbox['y2'] * (resized_height / float(height)) / 16))
