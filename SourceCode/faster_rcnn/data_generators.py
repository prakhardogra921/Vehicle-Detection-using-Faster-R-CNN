...
def calc_rpn(img_data, width, height, resized_width, resized_height, img_length_calc_function):
	...
	if len(pos_locs[0]) > num_regions/2:
		val_locs = sample(range(len(pos_locs[0])), len(pos_locs[0]) - num_regions/2)
		y_is_box_valid[0, pos_locs[0][val_locs], pos_locs[1][val_locs], pos_locs[2][val_locs]] = 0
		num_pos = num_regions/2

	if len(neg_locs[0]) + num_pos > num_regions:
		val_locs = sample(range(len(neg_locs[0])), len(neg_locs[0]) - num_pos)
...
def get_anchor_gt(all_img_data, class_count, img_length_calc_function, backend, mode='train'):
	img_channel_mean = [125.21, 125.36, 121.34]
	sample_selector = SampleSelector(class_count)
	while True:
		if mode == 'train':
			shuffle(all_img_data)
		...
		for img_data in all_img_data:
			try:
				...
				y_rpn_regr[:, y_rpn_regr.shape[1]//2:, :, :] *= 4.0
