...
from faster_rcnn import vgg as vn
...
def train():
    ...
    num_rois = 32
    epochs = 200
    model = "model_trained/frcnn_vgg.hdf5"
    ground_truth = "MIO-TCD-Localization/gt_train.csv"
    pretrained_model = "model/vgg16_weights_tf_dim_ordering_tf_kernels.h5"
    ...
    data_gen_train = get_anchor_gt(train_data, label_dict, vn.get_img_output_length, K.image_dim_ordering(), mode='train')
    data_gen_val = get_anchor_gt(val_data, label_dict, vn.get_img_output_length, K.image_dim_ordering(), mode='val')
    ...
    shared_layers = vn.nn_base(img_input, trainable=True)

    # define the RPN using the base layers
    num_anchors = 3 * 3 # 3 for number of scales and 3 number of aspect ratios
    rpn = vn.rpn(shared_layers, num_anchors)

    classifier = vn.classifier(shared_layers, roi_input, num_rois, nb_classes=len(label_dict), trainable=True)
