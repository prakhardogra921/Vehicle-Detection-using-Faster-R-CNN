...
def predict_single_image(img_path, model_rpn, model_classifier_only, class_mapping):
    ...
    for cls_num, box in boxes.items():
        ...
        for b in boxes_nms:
            ...
            f.write(",".join(
                [img_path.split("/")[-1].split(".")[0], class_mapping[cls_num], str(b[-1]), str(b[0]), str(b[1]),
                 str(b[2]), str(b[3])]) + "\n")

    img = draw_boxes_and_label_on_image_cv2(img, class_mapping, boxes)
    result_path = './vgg_results_images/{}.jpg'.format(os.path.basename(img_path).split('.')[0])
...
def predict(args_):
    ...
    rpn_layers = vn.rpn(shared_layers, num_anchors)
    classifier = vn.classifier(feature_map_input, roi_input, 32, nb_classes=len(class_mapping), trainable=True)
